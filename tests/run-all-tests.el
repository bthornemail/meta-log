...
(load (expand-file-name "test-quadratic-forms.el" (file-name-directory (or load-file-name default-directory))))
(load (expand-file-name "test-quaternion.el" (file-name-directory (or load-file-name default-directory))))
(load (expand-file-name "test-p-adic.el" (file-name-directory (or load-file-name default-directory))))
(load (expand-file-name "test-shimura-drinfeld.el" (file-name-directory (or load-file-name default-directory))))
(load (expand-file-name "test-geometric-alignments.el" (file-name-directory (or load-file-name default-directory))))
...
(defun run-all-tests ()
  ...
  (let ((start-time (current-time))
    ...
    (message "")
    (push (cons "Quadratic Forms Tests" (test-quadratic-forms-all)) results)
    (message "")
    (push (cons "Quaternion Tests" (test-quaternion-all)) results)
    (message "")
    (push (cons "p-Adic Tests" (test-p-adic-all)) results)
    (message "")
    (push (cons "Shimura-Drinfeld Tests" (test-shimura-drinfeld-all)) results)
    (message "")
    (push (cons "Geometric Alignments Tests" (test-geometric-alignments-all)) results)
    (message "")
